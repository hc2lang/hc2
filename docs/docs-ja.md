...
hc2 はメモリ安全なプログラミング言語です。すべてのポインタは、実行時に
検査される capability として扱われます。ガベージコレクタも借用チェッカーも
ありません。違反が起きた場合、プログラムの状態を破壊するのではなく、その場で停止します。

コンパイラは hc2 自身で書かれており、セルフコンパイルが可能です。
アセンブラとリンカも内蔵しているため、`as` も `ld` もlibc も不要です。
`linux/amd64` と `linux/arm64` 向けには静的リンクされたELFバイナリ、
`macos/arm64` 向けには署名済みの Mach-Oバイナリ を生成します。
...
I32 main() {
    ...
    "%c\n", view[0];     // C なら解放済みメモリを読んで動いてしまう。ここでは止まる
    return 0;
}
```
...
- [ポインタとcapability](#ポインタとcapability)
    - [スライス](#スライス)
    - [ポインタ演算](#ポインタ演算)
    - [null](#null)
- [配列](#配列)
...
| 2 | `.lo`, `.len` | 割り当て内の窓のオフセットと、値のバイト長 |
...
| `U0 free(U8* p)` | 割り当てを失効させ、ブロックを再利用に回す |
| `I64 mark()` | 現在の割り当て位置を記録する |
| `U0 release(I64 m)` | マークまで巻き戻し、それ以降の割り当てをすべて捨てる |

`heap` は 256 MiB の匿名マッピング 1 つの上のアロケータです。要求サイズは
2 の冪のクラスに切り上げられ、`free` されたブロックは同じクラスの次の
`alloc` で再利用されます。再利用できるブロックがなければ末尾から切り出し
ます。切り出したばかりのメモリはゼロ埋めされていて(初回タッチのページ)、
再利用されたブロックには前の内容が残っています。`alloc(0)` は合法、領域が
尽きると `heap: trap: out of memory` でトラップします。

### freeと失効

`heap.free` はまず割り当てを**失効**させます。各割り当てのヘッダは正式な
世代を持ち、各ポインタは自分が期待する世代を運びます。free はヘッダの
世代を 1 つ進め、その瞬間から、プログラムのどこにあるどのコピーもヘッダと
食い違い、次に使われたときにトラップします。ブロックが後で再利用されても
世代は 1 に戻らず、進んだ値を引き継ぐので、古いポインタは再利用後も
トラップします:
...
`release` は世代を進め**ません**(フリーリストは捨てます)。自分の領域の
`release` を生き延びたポインタは、次にそこへ割り当てられたものの別名に
なります。マークより上を
